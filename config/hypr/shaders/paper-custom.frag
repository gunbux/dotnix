...
// Author: khing, modified by gunbux
...
float paper_grain(vec2 uv){
    vec2 p = uv * 1024.0;
    float grain = fract(sin(dot(p, vec2(127.1, 311.7))) * 43758.5453123);
    grain += fract(sin(dot(p + vec2(0.5), vec2(269.5, 183.3))) * 17.0) * 0.5;
    grain = fract(grain);
    return (grain - 0.5) * 0.12; // Range: [-0.06, +0.06]
}
